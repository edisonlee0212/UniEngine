    ...
    for (std::vector<std::string>::size_type i = 0; i < current_dirlist.size(); i++)
    {
        ...
        if (i != current_dirlist.size() - 1)
        {
            ...
            else
            {
                ...
                ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
                ImGui::ArrowButtonEx(
                    "##Right", ImGuiDir_Right, ImVec2(frame_height, frame_height), ImGuiButtonFlags_Disabled);
                ImGui::SameLine(0, 0);
                ImGui::PopStyleColor(2);
            }
        }
...
bool ImGuiFileBrowser::renderFileListRegion()
{
    ...
    int num_cols = static_cast<int>(
        std::max(1.0f, std::ceil(static_cast<float>(filtered_dirs.size() + filtered_files.size()) / col_items_limit)));
    ...
    if (num_cols > 64)
    {
        ...
        num_cols = static_cast<int>(std::max(
...
bool ImGuiFileBrowser::renderInputTextAndExtRegion()
{
    ...
    if (ext_box_width < 0.0)
        ext_box_width =
            ImGui::CalcTextSize("All Valid Files").x + style.ItemSpacing.x + ImGui::GetFrameHeightWithSpacing() + 10;
...
bool ImGuiFileBrowser::renderButtonsAndCheckboxRegion()
{
    ...
    else
    {
        if (ImGui::Button("Open", ImVec2(button_width, 0)))
        {
            // It's possible for both to be true at once (user selected directory but input bar has some text. In this
            // case we chose to open the directory instead of opening the file. Also note that we don't need to access
            // the selected file through "selected_idx" since the if a file is selected, input bar will get populated
            // with that name.
...
bool ImGuiFileBrowser::readDIR(std::string pathdir)
{
    ...
    /* If the current directory doesn't exist, and we are opening the dialog for the first time, reset to defaults to
     * avoid looping of showing error modal. An example case is when user closes the dialog in a folder. Then deletes
     * the folder outside. On reopening the dialog the current path (previous) would be invalid.
     */
...
// Somehow there is a '..' present in root directory in linux.
    ...
    {
        error_title = "Error!";
        error_msg = "Error opening directory! Make sure the directory exists and you have the proper rights to access "
                    "the directory.";
        return false;
    }
    return true;
...
bool ImGuiFileBrowser::validateFile()
{
    ...
    // If there is an item selected, check if the selected file name (the input filename, in other words) matches the
    // selection.
    if (selected_idx >= 0)
    {
        if (dialog_mode == DialogMode::SELECT)
...
std::string ImGuiFileBrowser::wStringToString(const wchar_t *wchar_arr)
{
    ...
    // MinGW bug (patched in mingw-w64), wcsrtombs doesn't ignore length parameter when dest = nullptr. Hence the large
    // number.
