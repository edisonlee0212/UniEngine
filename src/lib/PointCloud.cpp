...
void PointCloud::OnGui()
{
    ...
    FileUtils::OpenFile(("Load PLY file##Particles"), ".ply", [&](const std::filesystem::path &filePath) {
        try
        {
            Load(filePath);
    ...
    FileUtils::SaveFile(("Save Compressed to PLY##Particles"), ".ply", [&](const std::filesystem::path &filePath) {
        try
        {
            Save(filePath);
