...
void RenderManager::RenderToCameraForward(const std::unique_ptr<CameraComponent> &cameraComponent)
{
	auto &renderManager = GetInstance();
...
void RenderManager::CollectRenderInstances(
	const GlobalTransform &cameraTransform, Bound& worldBound)
{
	auto &renderManager = GetInstance();
	...
	if (owners)
	{
		for (auto owner : *owners)
		{
			...
			renderInstance.m_renderer = mmc.get();
			renderInstance.m_type = RenderInstanceType::Default;
			if (mmc->m_material->m_blendingMode != MaterialBlendingMode::Off)
			{
				renderManager.m_transparentRenderInstances[distance].push_back(
					renderInstance);
			}
			else if (mmc->m_forwardRendering)
			{
				renderManager.m_forwardRenderInstances[mmc->m_material.get()][distance].push_back(
					renderInstance);
			}
			else
			{
				renderManager.m_deferredRenderInstances[mmc->m_material.get()][distance].push_back(
					renderInstance);
			}
		}
	}
	...
	if (owners)
	{
		for (auto owner : *owners)
		{
			...
			RenderInstance renderInstance;
			renderInstance.m_globalTransform = gt;
			renderInstance.m_renderer = particles.get();
			renderInstance.m_type = RenderInstanceType::Default;
			if (particles->m_material->m_blendingMode != MaterialBlendingMode::Off)
			{
				renderManager.m_instancedTransparentRenderInstances[distance].push_back(
					renderInstance);
			}
	...
	if (owners)
	{
		for (auto owner : *owners)
		{
			...
			RenderInstance renderInstance;
			renderInstance.m_globalTransform = gt;
			renderInstance.m_renderer = smmc.get();
			...
			{
				renderManager.m_forwardRenderInstances[smmc->m_material.get()][distance].push_back(
					renderInstance);
			}
			else
			{
				renderManager.m_deferredRenderInstances[smmc->m_material.get()][distance].push_back(
					renderInstance);
			}
		}
	}
...
void RenderManager::PreUpdate()
{
	...
	if (renderManager.m_mainCameraComponent)
	{
		CollectRenderInstances(
			renderManager.m_mainCameraComponent->GetOwner().GetComponentData<GlobalTransform>(), worldBound);
	}
	else
	{
...
#pragma region Render to scene camera
	if (editorManager.m_enabled && editorManager.m_sceneCamera->IsEnabled())
	{
		...
		cameraTransform.m_value =
			glm::translate(editorManager.m_sceneCameraPosition) * glm::mat4_cast(editorManager.m_sceneCameraRotation);
...
#pragma region For entity selection
		...
		if (immcowners)
		{
			...
			for (auto owner : *immcowners)
			{
				...
				mesh->Vao()->SetAttributeDivisor(14, 1);
				mesh->Vao()->SetAttributeDivisor(15, 1);
				editorManager.m_sceneCameraEntityInstancedRecorderProgram->SetInt("EntityIndex", owner.m_index);
...
#pragma endregion
		...
		RenderToCameraDeferred(editorManager.m_sceneCamera);
		RenderBackGround(editorManager.m_sceneCamera);
		RenderToCameraForward(editorManager.m_sceneCamera);
	}
#pragma endregion
	...
	if (renderManager.m_mainCameraComponent != nullptr)
	{
		...
		if (mainCameraEntity.IsEnabled())
		{
			...
			if (renderManager.m_mainCameraComponent->IsEnabled())
			{
				...
				RenderToCameraDeferred(mainCameraComponent);
				RenderBackGround(mainCameraComponent);
				RenderToCameraForward(mainCameraComponent);
	...
	const size_t &count)
{
	...
	skinnedMesh->Vao()->SetAttributePointer(14, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4), (void *)(2 * sizeof(glm::vec4)));
	skinnedMesh->Vao()->EnableAttributeArray(15);
	skinnedMesh->Vao()->SetAttributePointer(15, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4), (void *)(3 * sizeof(glm::vec4)));
	...
	glDrawElementsInstanced(GL_TRIANGLES, (GLsizei)skinnedMesh->GetTriangleAmount() * 3, GL_UNSIGNED_INT, 0, (GLsizei)count);
	OpenGLUtils::GLVAO::BindDefault();
}
