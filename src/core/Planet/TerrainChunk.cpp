...
glm::dvec3 Planet::TerrainChunk::ChunkCenterPosition(glm::dvec3 planetPosition, double radius, glm::quat rotation)
{
	...
	glm::dvec3 point = m_localUp + (percent.x + (double)m_chunkCoordinate.x / (double)actualDetailLevel - 0.5) * 2 * m_axisA + (percent.y + (double)m_chunkCoordinate.y / (double)actualDetailLevel - 0.5) * 2 * m_axisB;
...
void Planet::TerrainChunk::Expand(std::mutex& mutex)
{
	...
	if (!m_c0)
	{
		auto chunk0 = std::make_unique<TerrainChunk>(m_planetTerrain, this, m_detailLevel + 1, glm::ivec2(m_chunkCoordinate.x * 2, m_chunkCoordinate.y * 2 + 1), ChunkDirection::UpperLeft, m_localUp);
		GenerateTerrain(mutex, chunk0);
		m_c0 = std::move(chunk0);
	}
	if (!m_c1)
	{
		auto chunk1 = std::make_unique<TerrainChunk>(m_planetTerrain, this, m_detailLevel + 1, glm::ivec2(m_chunkCoordinate.x * 2 + 1, m_chunkCoordinate.y * 2 + 1), ChunkDirection::UpperRight, m_localUp);
		GenerateTerrain(mutex, chunk1);
		m_c1 = std::move(chunk1);
	}
	if (!m_c2)
	{
		auto chunk2 = std::make_unique<TerrainChunk>(m_planetTerrain, this, m_detailLevel + 1, glm::ivec2(m_chunkCoordinate.x * 2, m_chunkCoordinate.y * 2), ChunkDirection::LowerLeft, m_localUp);
		GenerateTerrain(mutex, chunk2);
		m_c2 = std::move(chunk2);
	}
	if (!m_c3)
	{
		auto chunk3 = std::make_unique<TerrainChunk>(m_planetTerrain, this, m_detailLevel + 1, glm::ivec2(m_chunkCoordinate.x * 2 + 1, m_chunkCoordinate.y * 2), ChunkDirection::LowerRight, m_localUp);
...
void Planet::TerrainChunk::GenerateTerrain(std::mutex& mutex, std::unique_ptr<TerrainChunk>& targetChunk) const
{
	...
	for (auto index = 0; index < size; index++) {
		...
		glm::dvec2 globalPercent = 45.0 * glm::dvec2((percent.x + (double)targetChunk->m_chunkCoordinate.x / actualDetailLevel - 0.5) * 2.0, (percent.y + (double)targetChunk->m_chunkCoordinate.y / actualDetailLevel - 0.5) * 2.0);
		glm::dvec2 actualPercent = glm::dvec2(glm::tan(glm::radians(globalPercent.x)), glm::tan(glm::radians(globalPercent.y)));
		glm::dvec3 pointOnUnitCube = targetChunk->m_localUp + actualPercent.x * targetChunk->m_axisA + actualPercent.y * targetChunk->m_axisB;
...
{
	if (!m_c0 || !m_c1 || !m_c2 || !m_c3) return;
	if (!m_c0->Active || !m_c1->Active || !m_c2->Active || !m_c3->Active) return;
	m_c0->Active = false;
	m_c1->Active = false;
	m_c2->Active = false;
	m_c3->Active = false;
