...
class UNIENGINE_API ImGuiFileBrowser
{
  public:
     ...
     * the size, a DialogMode enum value defining in which mode the dialog should operate and optionally the extensions
     * that are valid for opening. Note that the select directory mode doesn't need any extensions.
     */
    bool showFileDialog(
    ...
    /* Store the opened/saved file name or dir name (incase of selectDirectoryDialog) and the absolute path to the
     * selection Should only be accessed when above functions return true else may contain garbage.
     */
  ...
  private:
    ...
    bool show_hidden, show_inputbar_combobox, is_dir, is_appearing, filter_dirty, validate_file,
        show_files_with_valid_extensions, show_all_files;
    char input_fn[256];
    ...
    std::vector<const Info *> filtered_dirs; // Note: We don't need to call delete. It's just for storing filtered items
                                             // from subdirs and subfiles so we don't use PassFilter every frame.
